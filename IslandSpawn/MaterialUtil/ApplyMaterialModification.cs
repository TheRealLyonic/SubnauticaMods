...
namespace LyonicDevelopment.IslandSpawn.MaterialUtil
{
    ...
    public class ApplyMaterialModification : MonoBehaviour
    {
        ...
        [SerializeField] private TextureSetting[] textureProperties = Array.Empty<TextureSetting>();
        [SerializeField] private ColorSetting[] colorProperties = Array.Empty<ColorSetting>();
        [SerializeField] private FloatSetting[] floatProperties = Array.Empty<FloatSetting>();
        [SerializeField] private EnableSetting[] enableProperties = Array.Empty<EnableSetting>();
        [SerializeField] private KeywordSetting[] keywordProperties = Array.Empty<KeywordSetting>();
        [SerializeField] private VectorSetting[] vectorProperties = Array.Empty<VectorSetting>();
        ...
        public static void ApplyAllModifications(GameObject rootObject)
        {
            foreach (var modifier in rootObject.GetComponentsInChildren<ApplyMaterialModification>())
            {
                foreach (var material in modifier.renderer.materials)
                {
                    if(material.name.ToLower().Contains("glass"))
                        MaterialUtils.SetMaterialTransparent(material, true);
                }
